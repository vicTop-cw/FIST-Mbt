...
> **一句话定位**：纯 MoonBit 的 AI 指挥官任务编排底座 + MCP Server + 自进化——"人类指挥、AI/定时器持续自推动"的自治系统，以 95 个 MCP 工具暴露给任意 MCP 客户端。
...
┌───────────────────────────▼───────────────────────────────────────┐
│  ① 协议层  src/server/       server.mbt     95 工具注册 + run_server │
...
1. **纯 MoonBit，零运行时依赖**：无 Python/Rust 包装；JS + Native 双端交叉编译，Windows/Linux 各 273 项测试全绿、跨环境可复现（`moon update && moon run cmd/main` 即用）。
...
- 95 工具分组：`README.md` §MCP 暴露面（生命周期/查询/DAG/自驱/演化/运维/Omega/调用日志+bug上报）
