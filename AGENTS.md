...
> **JS 与 Native 双后端均已在 Windows + WSL(Linux) 通过 209/209 测试。**
...
- **native 目标**：需要系统 SQLite 开发库（`sqlite3.h` + `sqlite3.lib`）。Windows + MSVC 下：
  ...
  Windows native **并行**跑全量测试偶发 `0xc0000374`（堆损坏/竞态），建议 `moon test --target native -j 1` 串行（稳定 209/209），见 README「已知边界」。

## MCP Server

本项目通过 `.mcp.json` 暴露 `fist-mbt` MCP Server（**75 tools** + 3 resources + 2 prompts）：
...
| 工具 | 说明 |
|---|---|
| `task_plan_deep` | AO 式递归拆解（可选 `gradient=true` 使子任务带难度梯度与更简单变体提示） |
