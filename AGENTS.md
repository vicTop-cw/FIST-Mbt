...
> **JS 与 Native 双后端均已在 Windows + WSL(Linux) 通过 205/205 测试。**
...
- **native 目标**：需要系统 SQLite 开发库（`sqlite3.h` + `sqlite3.lib`）。Windows + MSVC 下：
  ...
  Windows native **并行**跑全量测试偶发 `0xc0000374`（堆损坏/竞态），建议 `moon test --target native -j 1` 串行（稳定 205/205），见 README「已知边界」。
