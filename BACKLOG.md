...
> 状态枚举 `pending | done`。锚点事实：**103 工具 / 307 测试**（`moon test --target js` Windows+WSL 双端全绿，含 quickcheck 属性测试）/ JS+Native 双后端 / CI 三绿 / evolve 已在测试内。计数同步见 `scripts/check_tools_sync.py` / `check_test_sync.py`。
