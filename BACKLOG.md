...
> 状态枚举 `pending | done`。锚点事实：**104 工具 / 311 测试**（`moon test --target js` Windows+WSL 双端全绿，含 quickcheck 属性测试）/ JS+Native 双后端 / CI 三绿 / evolve 已在测试内。计数同步见 `scripts/check_tools_sync.py` / `check_test_sync.py`。
...
| P2 | 集成 moonbitlang/core/quickcheck 属性测试替代部分硬编码断言 | competition/§四 P2-8 + five-directions/§六 + future-roadmap/中 | done(R107 已落地：core/decompose 两个此前无直接测试的包补 8 条 quickcheck 属性——default_slices 数量与前缀 / difficulty_label 三档单调 / difficulty_label_from 单调 / Task 迁移纪律 claim/execute/reopen/split/submit/reject，固定 seed 可复现 + OOM 域防护；test-only 导入不污染产物体积；287→295/295) | - |
...
| P2 | Evaluator-Optimizer schema：feedback 收敛为 Defects/Evidence/Fix/Acceptance 四段式 | five-directions/§一 | done(eval_feedback 已落地：自由文本反馈归一为四段式契约 + 确定性 verdict（无缺陷且 acceptance 非空 → pass 可收敛；否则 fail + 缺证据/缺修复/缺段标注），纯读不写库与 plan_revise 互补；R111 见调研档 20260926.evaluator-optimizer-schema.md) | - |
...
| P2 | Circuit Breaker 三态：Closed/Open/Half-Open 对外部调用快速失败 | five-directions/§一 | done(circuit_fail/circuit_succeed/circuit_status 已落地：Closed 窗口失败计数达阈值 → Open fail-fast / 恢复定时器 → Half-Open 探测 / 成功 Closed 失败回 Open，双后端 circuit_breakers 表；R104 见调研档 20260926.circuit-breaker.md) | - |
...
| P3 | 补充 English README（面向 Lambda World 2026 国际受众） | competition/§四 P3-12 + future-roadmap/近 | done(README_EN.md 已落地：103 工具/307 测试/机制家族/SRE 四金信号等全量英文呈现，评审首读与 Lambda World 2026 国际受众双受益；R106) | - |
...
| P3 | Saga 补偿事务 + durable action log：并发动作前写 append-only 日志、失败按 LIFO 补偿 | five-directions/§一 | done(saga_register/saga_rollback 已落地：saga_log 表 + LIFO 幂等补偿序列，R92 见调研档 20260926.saga-compensation.md) | - |
