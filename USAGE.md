...
> `PASS tools/list → 76 个工具` / `PASS publish → T?` / `PASS get → T? [待领取]` / `MCP-SMOKE PASS`。
> 三步 = 该脚本的内部逻辑，二者完全一致。
...
> 补充实测：`tools/list` 返回 76 个工具；`resources/read(fist://principles)` 返回七条金条 JSON；
...
FIST-Mbt = 用纯 MoonBit 实现的 FIST 指挥官任务编排 + MCP STDIO Server（76 个工具）。
