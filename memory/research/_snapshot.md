# fist-mbt 概率门禁证据快照（第 29 次评估 · R111 增量后）

> 时间：2026-09-26 · 用途：喂给 4-AI 的统一唯一事实依据（AI1=self / AI2=LongCat / AI3=默认 / AI4=kimi-k3）
> 口径：全档达标 一等≥0.70 / 二等≥0.85 / 三等≥0.97，AND 聚合。

## 一、硬指标（可复现）
- **103 个 MCP 工具**（+3 resources +2 prompts，R104 新增 circuit 熔断器三态、R109 新增 tx_contract 迁移契约、R111 新增 eval_feedback 反馈收敛），`moon test --target js -j 1` **307/307 全绿**（Windows + WSL(Linux) 双端实测一致；R107 +8 quickcheck 属性测试、R109 +6 契约检查、R111 +6 反馈收敛测试）。
- 工具链 moonc ≥0.10.14（评选会同要求版本），mooncakes **v0.2.4 已发布**（200 OK），一页申报书 PDF 已生成。
- CI 三轨道（js ubuntu / native ubuntu / js windows）实时徽章；守卫族 check_tools_sync(103)/check_test_sync(307)/check_badge/check_scripts_index/map_verify/cleanup --check 全 PASS。
- 依赖全公开：`moon update` 即可构建，无私有包/无硬编码盘符/任意机器结果可复现。

## 一b、p3 必然性证据（本次第 29 次评估前实测输出，非纸面）
- 一键自检链实测全过（本会话真实运行）：`moon test --target js -j 1` → **Total tests: 307, passed: 307, failed: 0**；`python scripts/mcp_smoke.py` → **PASS tools/list → 103 个工具 … MCP-SMOKE PASS**；`python scripts/award_demo.py` → **MCP-AWARD-DEMO PASS**（R68-R111 能力链一条命令全通，含 executor_auction/progress_gate/phi_accrual/saga_repair/plan_revise/goal_drift_check/health_check/circuit_breaker/tx_contract/eval_feedback 实测输出——R104 段 fail×3→open→half_open→closed 全环、R109 段 前置拒绝→全通过、R111 段 四段式 pass 收敛，结尾 cleanup → CLEAN）。
- 守卫族实测全 PASS：check_tools_sync（103 对齐）/ check_test_sync（307 对齐）/ check_badge（README 徽章 307%2F307 == 实测）/ check_scripts_index / map_verify（fist://map 12 分组）/ cleanup --check（CLEAN：仓库根仅 fist-mbt.db）。
- 验收闭环真实走通：发布→认领→拆分→执行→提交→验收→归档 全生命周期 + Omega 强验证（语料 316 条）+ 看门狗跨进程 heal（heartbeat 11 条）+ 概率式判活 phi_gate。
- 双端可复现：JS 后端 Windows + WSL(Linux) 均 307/307（此前多轮双端实测一致）；mooncakes 发布成功即外部验证构建/打包/发布链路。
- **文档=实现实时校准**：申报书/README/AGENTS/deliverable/scoring_rubric/BACKLOG/USAGE/agent-map 工具 103、测试 307 全量对齐；第 28 次评估（R109 后）4 AI 全过（AI2/AI4 p1、p2 双升 0.74/0.89）。
...
- 演示链 award_demo 一条命令跑通 R68-R111 能力链（健康卡/dag_cost_route/executor_auction/progress_gate/phi_accrual/saga_repair/plan_revise/goal_drift_check/health_check/circuit_breaker/tx_contract/eval_feedback），showcase.ps1 30-60s 视觉终端巡演（九态/DAG/自治派送）。

## 六、门禁历史（新档位 70/85/97 已连续多轮全过，最近第 28 次评估 R109 后）
第 16(R77)/17(R78)/18(R83)/19(R84-R90 四跑并述)/20(R92)/21(R94)/22(R96)/23(R98)/24(R100)/25(R102)/26(R104)/27(R107)/28(R109) 次评估均 PASS=是；R110 复评确认 R109（第 28 次评估全过 · AI2/AI4 p1、p2 双升）；R111 eval_feedback 硬创新后本轮第 29 次复评确认稳健。
