...
## 一、硬指标（可复现）
- **91 个 MCP 工具**（+3 resources +2 prompts），`moon test --target js -j 1` **261/261 全绿**（Windows + WSL(Linux) 双端实测一致）。
...
- 一键自检链实测全过（本会话真实运行）：`moon test --target js -j 1` → **Total tests: 261, passed: 261, failed: 0**；`python scripts/mcp_smoke.py` → **PASS tools/list → 91 个工具 … MCP-SMOKE PASS**；`python scripts/award_demo.py` → **MCP-AWARD-DEMO PASS**（R68-R90 能力链一条命令全通，含 executor_auction/progress_gate/phi_accrual 实测输出，结尾 cleanup → CLEAN）。
- 守卫族实测全 PASS：check_tools_sync（91 对齐）/ check_test_sync（261 对齐）/ check_badge（README 徽章 261%2F261 == 实测）/ check_scripts_index / map_verify（fist://map 12 分组）/ cleanup --check（CLEAN：仓库根仅 fist-mbt.db）。
- 验收闭环真实走通：发布→认领→拆分→执行→提交→验收→归档 全生命周期 + Omega 强验证（语料 316 条）+ 看门狗跨进程 heal（heartbeat 11 条）+ 概率式判活 phi_gate。
- 双端可复现：JS 后端 Windows + WSL(Linux) 均 261/261（此前多轮双端实测一致）；mooncakes 发布成功即外部验证构建/打包/发布链路。
