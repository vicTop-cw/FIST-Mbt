...
**基础 / 演示**
- `mcp_smoke.py` — MCP server 一键自检（79 工具 + publish/get 链路）。
...
- `evolve_critic_verify.py` — `evolve_critic` Critic 防漂移门禁 E2E（79 工具 + 放行/拒收/收紧阈值 + 只评审不写库）。
- `task_challenge_verify.py` — `task_challenge` Challenger 进阶变体 E2E（79 工具 + [challenge]溯源/扩规模 + 未完成任务拒绝 + scratch 隔离）。
