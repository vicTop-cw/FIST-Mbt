...
**基础 / 演示**
- `mcp_smoke.py` — MCP server 一键自检（82 工具 + publish/get 链路）。
...
- `evolve_critic_verify.py` — `evolve_critic` Critic 防漂移门禁 E2E（82 工具 + 放行/拒收/收紧阈值 + 只评审不写库）。
- `task_challenge_verify.py` — `task_challenge` Challenger 进阶变体 E2E（82 工具 + [challenge]溯源/扩规模 + 未完成任务拒绝 + scratch 隔离）。
- `executor_route_verify.py` — **执行者能力路由（Marketplace 雏形）E2E**：`executor_register` 登记能力 + `executor_route` 按「能力覆盖率 desc → 负载 asc」路由最佳执行者（82 工具）；R31 跨进程三步：进程A注册→进程B路由读到→clear 消失。
