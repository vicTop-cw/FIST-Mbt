...
# 期望输出：PASS tools/list → 93 个工具 / PASS publish / PASS get → MCP-SMOKE PASS
```

## 二、硬指标（快照）
| 项 | 值 |
|---|---|
| MCP 工具 | **93**（+ 3 resources + 2 prompts） |
| 测试 | **`moon test --target js` 265/265**（Windows + WSL(Linux) 双端实测全绿） |
