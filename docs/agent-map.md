...
- **构建/测试**：`moon check` / `moon test --target js`（219 项全绿）
...
| `server` | MCP 层：78 个工具+3 resources+2 prompts 注册、call_log 注入、Laya/evolve MCP 封装 | `server.mbt` `laya_js.mbt` `evolve_distill.mbt` `evolve_lesson.mbt` |
| `decompose` | 递归拆解规格 | — |
| `executor` | 执行器 | — |
