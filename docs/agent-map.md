...
- **构建/测试**：`moon check` / `moon test --target js`（316 项全绿）
...
- **找目标项目潜问题**：`issue_scan`（MCP 工具）/ `python scripts/issue_scan.py <dir>`（CLI）/ [技能文档](issue-scan-skill.md)（skill）三形态，规则驱动扫描，命中喂 `report_bug` 闭环
...
| `server` | MCP 层：104 个工具+3 resources+2 prompts 注册、call_log 注入、Laya/evolve MCP 封装、规则驱动源码扫描（issue_scan） | `server.mbt` `laya_js.mbt` `evolve_distill.mbt` `evolve_lesson.mbt` `issue_scan.mbt` |
| `decompose` | 递归拆解规格 | — |
...
| 运维·日志/缺陷/成本/调度(12) | call_log/bug_list/report_bug/run_check/issue_scan/schedule/pipeline_tick/cost_stats/cost_budget_check/cost_budget_split/progress_gate/laya_decide |
