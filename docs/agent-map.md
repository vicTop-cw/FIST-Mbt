...
| `server` | MCP 层：68 个工具+3 resources+2 prompts 注册、call_log 注入、Laya/evolve MCP 封装 | `server.mbt` `laya_js.mbt` `evolve_distill.mbt` |
| `decompose` | 递归拆解规格 | — |
| `executor` | 执行器 | — |
...
| DAG(6) | dag_critical_path/parallelism/ascii/check/ready/sort |
| 自驱(8) | selfdrive_init/append/get/export_tasks/review_tick/review_ready/publish_next/parse_next_tasks |
| 记忆/自进化(4+) | memory_consolidate/gc/link、evolve_submit/sample/snapshot/distill/asset_register、self_search |
