...
| `server` | MCP 层：81 个工具+3 resources+2 prompts 注册、call_log 注入、Laya/evolve MCP 封装 | `server.mbt` `laya_js.mbt` `evolve_distill.mbt` `evolve_lesson.mbt` |
| `decompose` | 递归拆解规格 | — |
| `executor` | 执行器抽象 + 能力注册表 + 能力路由（Marketplace 雏形，R30） | `registry.mbt` `base.mbt` |
...
| Marketplace·能力路由(2) | executor_register、executor_route（执行者能力登记 + 按专长/负载路由，R30） |
