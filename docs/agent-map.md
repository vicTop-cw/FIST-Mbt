...
| `server` | MCP 层：95 个工具+3 resources+2 prompts 注册、call_log 注入、Laya/evolve MCP 封装 | `server.mbt` `laya_js.mbt` `evolve_distill.mbt` `evolve_lesson.mbt` |
| `decompose` | 递归拆解规格 | — |
...
| 看板/脉冲/预订/推荐+DAG(17) | dag_critical_path/parallelism/ascii/check/ready/sort/depend/publish/slack/schedule/cost_route/mc、board_ascii、status_summary、reserve_scope/check/release、task_triage |
