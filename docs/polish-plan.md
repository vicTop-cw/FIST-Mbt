...
凡与目标项目相关的功能都应先调研（`memory/research/`）再迭代；每轮收敛前跑 4-AI 概率门禁，未全档达标不进下轮。
...
> 质量标准（主/次）：主为先决硬门槛（不达标即打回重迭代），次为须尽力保持的工程纪律；冲突时先保「主」。

**主 · 阻断性硬门槛（不达标即打回重迭代）**
1. **无硬编码**：源码/脚本/测试无盘符、`/home`、`/Users` 等绝对路径；路径相对或参数化（环境变量/参数注入）。
2. **无任何回归**：既有行为与全部测试语义不变；改动不破坏既有功能（当前双端全绿数 148）。
3. **测试全绿**：`moon test --target js` 与 `--target native`（Windows 用 `-j 1`）双端全绿（148/148）。
4. **功能兼容**：不改变既有 MCP 工具的参数/返回/语义（新增可，破坏不可）。
5. **任意机器结果可完全复现**：干净机器上 `moon update && moon run/test` 即得同等结果；不依赖本机私有路径/个人会话状态。

**次 · 期望性工程纪律（非阻断，失分项）**
6. **文档即实现**：README/USAGE/AGENTS/docs 与实际行为一致；改实现必同步文档，反之亦然。当「功能增量」与「主标准」冲突时，以主标准为准——宁可在功能上让步，不牺牲可复现与兼容。
